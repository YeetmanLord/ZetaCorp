...
public class InputParser {
    ...
    public static double parseMoney(String input) {
        if (input.contains("k") || input.contains("m") || input.contains("b") || input.contains("t")) {
            return parseLong(input.replace("$", ""));
        }
        input = input.toLowerCase().replace(" ", "").replace("$", "").replace(",", "").replace("$", "").replace("k", "000").replace("m", "000000").replace("b", "000000000").replace("t", "000000000000");;
        if (input.contains("e")) {
    ...
    public static long parseLong(String input) {
        input = input.toLowerCase().replace(" ", "").replace(",", "").replace("k", "000").replace("m", "000000").replace("b", "000000000").replace(".", "").replace("t", "000000000000");
        if (input.contains("e")) {
            String[] split = input.split("e");
